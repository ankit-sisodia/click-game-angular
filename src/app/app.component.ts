...
export class AppComponent implements OnInit {
  ...
  constructor( ) { }
  ngOnInit() {
    this.items = new Array(this.mapDiffArr[this.diff]);
    this.currentHighScore = this.highScore[this.diff] || 0;
    setInterval(() => {
      this.highlight = Math.floor((Math.random() * this.mapDiffArr[this.diff]));
      if (this.timer > 0) {
        this.timer--;
      }
      if ( this.timer === 0 ) {
        const click = confirm('Game Over!!!');
        if (click) {
          this.restartHandler();
        }
      }
